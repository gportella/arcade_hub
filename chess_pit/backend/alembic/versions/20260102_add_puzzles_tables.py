...
def upgrade() -> None:
    ...
    op.create_index("ix_puzzle_cool_id", PUZZLE_TABLE, ["cool_id"], unique=True)
    op.create_index("ix_puzzle_difficulty", PUZZLE_TABLE, ["difficulty"])
    ...
    op.create_table(
        ...
        sa.Column("solved", sa.Boolean(), nullable=False, server_default=sa.sql.expression.false()),
        sa.Column("hint_count", sa.Integer(), nullable=False, server_default="0"),
    ...
    op.create_index("ix_puzzleattempt_user_id", PUZZLE_ATTEMPT_TABLE, ["user_id"])
